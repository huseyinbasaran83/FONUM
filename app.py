...
# Sayfa Ayarları
st.set_page_config(page_title="Zenith Pro: Hatasız Analiz", layout="wide")
...
with st.sidebar:
    ...
    f_live = st.number_input("Anlık Birim Fiyat (TL)", min_value=0.000001, value=f_cost, format="%.6f")
...
# --- 5. ANA EKRAN ---
st.title("🛡️ Zenith Pro: Hassas Portföy Takibi")
...
if st.session_state.portfolio:
    ...
    h1.write("**Fon**"); h2.write("**Adet**"); h3.write("**Maliyet**"); h4.write("**Güncel Fiyat**"); h5.write("**Tarih**"); h6.write("**Sil**")

    # Satırlar
    for idx, item in enumerate(st.session_state.portfolio):
        c1, c2, c3, c4, c5, c6 = st.columns([0.8, 1, 1.2, 1.2, 1.3, 0.4])
        
        # Hata önleme: Eğer eski veride guncel_fiyat yoksa maliyeti kullan
        current_val = item.get('guncel_fiyat', item['maliyet'])
        
        with c1: st.write(f"**{item['kod']}**")
        with c2: st.session_state.portfolio[idx]['adet'] = st.number_input("", value=float(item['adet']), key=f"q_{idx}", format="%.6f", label_visibility="collapsed")
        with c3: st.session_state.portfolio[idx]['maliyet'] = st.number_input("", value=float(item['maliyet']), key=f"m_{idx}", format="%.6f", label_visibility="collapsed")
        with c4: st.session_state.portfolio[idx]['guncel_fiyat'] = st.number_input("", value=float(current_val), key=f"g_{idx}", format="%.6f", label_visibility="collapsed")
        with c5: 
            st.session_state.portfolio[idx]['tarih'] = st.date_input("", value=item['tarih'], key=f"d_{idx}", label_visibility="collapsed")
        with c6:
    ...
    if 'guncel_fiyat' not in df.columns: df['guncel_fiyat'] = df['maliyet']
    
    df['G_Deger'] = df['adet'] * df['guncel_fiyat']
    df['T_Maliyet'] = df['adet'] * df['maliyet']
    
    t1, t2 = st.tabs(["📈 Performans Analizi", "💎 Varlık Dağılımı"])
    ...
    with t1:
        ...
        st.dataframe(df[['kod', 'tarih', 'maliyet', 'guncel_fiyat', 'Kar/Zarar %', 'USD Fark %', 'Altın Fark %']].style.format({
            'maliyet': '{:.6f}', 'guncel_fiyat': '{:.6f}',
            'Kar/Zarar %': '% {:.2f}', 'USD Fark %': '% {:.2f}', 'Altın Fark %': '% {:.2f}'
        }).background_gradient(cmap='RdYlGn', subset=['Kar/Zarar %', 'USD Fark %', 'Altın Fark %']), use_container_width=True)

    with t2:
        all_hisseler = []
        for _, row in df.iterrows():
            comp = KAP_DATA.get(row['kod'], {f"{row['kod']}": 1.0})
        ...
        with cp: st.plotly_chart(px.pie(report_df, values='Değer', names='Varlık', hole=0.4), use_container_width=True)
        with cl: st.dataframe(report_df.style.format({'Değer': '{:,.2f} ₺', 'Yüzde (%)': '% {:.2f}'}), use_container_width=True)

    # ÖZET METRİKLER
    total_val = df['G_Deger'].sum()
    total_cost = df['T_Maliyet'].sum()
    m1, m2, m3 = st.columns(3)
    m1.metric("Portföy Değeri", f"{total_val:,.2f} ₺")
    ...
    st.info("İşlem ekleyerek başlayın.")
